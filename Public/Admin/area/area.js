...
function changem(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(m[i]).selectedIndex-1));
	};
	var ss=document.getElementById(m[v]);
	with(ss){
		length = 0;
		options[0]=new Option(opt01[v],opt01[v]);
		if(v && document.getElementById(m[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
...
var m=["m_province","m_city","m_county"];//三个select的name
var opt01 = ["省份","地级市","市、县级市"];//初始值
function _init_aream(){  //初始化函数
	for(var i=0;i<m.length-1;i++){
	  document.getElementById(m[i]).onchange=new Function("changem("+(i+1)+")");
	}
	changem(0);
}
